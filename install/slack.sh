...
# Run as: sudo bash install/slack.sh
...
main() {
  ...
  apt_config_url="https://packagecloud.io/install/repositories/slacktechnologies/slack/config_file.list?os=${os}&dist=${dist}&source=script"

  apt_source_path="/etc/apt/sources.list.d/slacktechnologies_slack.list"
  apt_keyrings_dir="/etc/apt/keyrings"
  if [ ! -d "$apt_keyrings_dir" ]; then
    install -d -m 0755 "$apt_keyrings_dir"
  fi

  gpg_keyring_path="$apt_keyrings_dir/slacktechnologies_slack-archive-keyring.gpg"
  gpg_key_path_old="/etc/apt/trusted.gpg.d/slacktechnologies_slack.gpg"
  ...
  echo "Running sudo apt install slack-desktop... "
